...
export const mail = {
	async welcome(m: Mailer, cfg: Config, to: string) {
		await send(
			...
			"Welcome",
			"Welcome to Vaultur",
			`<p>Your new account has been created! You can now log in and start storing passwords securely.</p>${btn(cfg.domain, "Open the web vault")}`,
			cfg.domain
		)
	},
	...
	async welcomeMustVerify(m: Mailer, cfg: Config, to: string, userId: string, token: string) {
		...
		await send(
			...
			"Welcome",
			"Welcome to Vaultur",
			`<p>Your new account has been created. Verify your email address to activate your account.</p>${btn(url, "Verify Email Address Now")}`,
			cfg.domain
		)
	},
	...
	async verifyEmail(m: Mailer, cfg: Config, to: string, userId: string, token: string) {
		...
		await send(
			...
			`<p>Verify this email address for your account by clicking the link below.</p>${btn(url, "Verify Email Address Now")}`,
			cfg.domain
		)
	},
	...
	async registerVerifyEmail(m: Mailer, cfg: Config, to: string, token: string) {
		...
		await send(
			...
			`<p>To finish creating your account, verify this email address by clicking the link below. This link expires in 30 minutes.</p>${btn(url, "Verify email")}<p>If you did not request this, you can safely ignore this email.</p>`,
			cfg.domain
		)
	},

	async newDeviceLoggedIn(
		m: Mailer,
		cfg: Config,
		to: string,
		deviceName: string,
		...
		await send(
			m,
			to,
			`New Device Logged In From ${deviceName}`,
...
<p>You can deauthorize all devices that have access to your account from the web vault under Settings &gt; My Account &gt; Deauthorize Sessions.</p>`,
			cfg.domain
		)
	},

	async incomplete2faLogin(
		m: Mailer,
		cfg: Config,
		to: string,
		deviceName: string,
		...
		await send(
			m,
			to,
			"Incomplete Two-Step Login From New Device",
			"Incomplete two-step login",
			`<p>Someone attempted to log into your account with the correct master password, but did not provide the correct token or action required to complete two-step login.</p>
<ul><li><b>Date:</b> ${dt}</li><li><b>IP Address:</b> ${ip}</li><li><b>Device:</b> ${deviceName}</li></ul>
<p>If this was not you, change your master password as soon as possible.</p>`,
			cfg.domain
		)
	},
	...
	async twofactorEmail(m: Mailer, cfg: Config, to: string, token: string, deviceIp: string) {
		await send(
			m,
			to,
			"Vaultur Login Verification Code",
...
<p>Use this code to complete logging in (request from ${deviceIp}). The code is valid for 10 minutes.</p>`,
			cfg.domain
		)
	},
	...
	async changeEmail(m: Mailer, cfg: Config, to: string, token: string) {
		await send(
			...
			`<p>To finalize changing your email address, enter the following token in the web vault:</p>
<p style="text-align:center;font-size:28px;letter-spacing:6px;font-weight:bold;">${token}</p>
<p>If you did not try to change your email address, contact your administrator.</p>`,
			cfg.domain
		)
	},
	...
	async changeEmailExisting(m: Mailer, cfg: Config, to: string, newEmail: string) {
		await send(
			...
			`<p>A request was just made to change your account's email address to <b>${newEmail}</b>.</p>
<p>If you did not make this request, contact your administrator immediately.</p>`,
			cfg.domain
		)
	},
	...
	async deleteAccount(m: Mailer, cfg: Config, to: string, userId: string, token: string) {
		...
		await send(
			...
			"Delete Your Account",
			"Delete your account",
			`<p>Click the link below to delete your account.</p>${btn(url, "Delete Your Account")}<p>If you did not request this email, you can safely ignore it.</p>`,
			cfg.domain
		)
	},
	...
	async orgInvite(
		...
		await send(
			...
			`Join ${orgName}`,
			`Join ${orgName}`,
			`<p>You have been invited to join the <b>${orgName}</b> organization.</p>${btn(url, "Join Organization Now")}<p>If you do not wish to join this organization, you can safely ignore this email.</p>`,
			cfg.domain
		)
	},
	...
	async inviteAccepted(m: Mailer, cfg: Config, to: string, invitedEmail: string, orgName: string) {
		await send(
			m,
			to,
			`Invitation to ${orgName} accepted`,
			"Invitation accepted",
			`<p>Your invitation for <b>${invitedEmail}</b> to join <b>${orgName}</b> was accepted. Please confirm the user from the organization members page.</p>`,
			cfg.domain
		)
	},
	...
	async inviteConfirmed(m: Mailer, cfg: Config, to: string, orgName: string) {
		await send(
			m,
			to,
			`Invitation to ${orgName} confirmed`,
			"Invitation confirmed",
			`<p>Your invitation to join <b>${orgName}</b> was confirmed. It will now appear under the Organizations section in the web vault.</p>`,
			cfg.domain
		)
	},

	async emergencyAccessInvite(
		m: Mailer,
		cfg: Config,
		to: string,
		emerId: string,
		...
		await send(
			m,
			to,
			`Emergency access contact request from ${grantorName}`,
			"Emergency access invitation",
			`<p><b>${grantorName}</b> has invited you to become an emergency access contact.</p>${btn(url, "Become emergency access contact")}<p>If you do not wish to accept, you can safely ignore this email.</p>`,
			cfg.domain
		)
	},
	...
	async emergencyAccessInviteAccepted(m: Mailer, cfg: Config, to: string, granteeEmail: string) {
		await send(
			m,
			to,
			"Accepted Emergency Access",
			"Emergency access accepted",
			`<p><b>${granteeEmail}</b> has accepted your emergency access invitation. Confirm them from your emergency access settings.</p>`,
			cfg.domain
		)
	},
	...
	async emergencyAccessInviteConfirmed(m: Mailer, cfg: Config, to: string, grantorName: string) {
		await send(
			m,
			to,
			"Confirmed as Emergency Access Contact",
			"Emergency access confirmed",
			`<p>You have been confirmed as an emergency access contact for <b>${grantorName}</b>.</p>`,
			cfg.domain
		)
	},

	async emergencyAccessRecoveryInitiated(
		m: Mailer,
		cfg: Config,
		to: string,
		granteeName: string,
		...
		await send(
			...
			"Emergency access initiated",
			`<p><b>${granteeName}</b> has initiated emergency access to <b>${atype}</b> your account. You have ${waitDays} day(s) to reject this request before access is automatically approved.</p>`,
			cfg.domain
		)
	},
	...
	async emergencyAccessRecoveryApproved(m: Mailer, cfg: Config, to: string, grantorName: string) {
		await send(
			...
			"Emergency access approved",
			`<p><b>${grantorName}</b> has approved your emergency access request.</p>`,
			cfg.domain
		)
	},
	...
	async emergencyAccessRecoveryRejected(m: Mailer, cfg: Config, to: string, grantorName: string) {
		await send(
			...
			"Emergency access rejected",
			`<p><b>${grantorName}</b> has rejected your emergency access request.</p>`,
			cfg.domain
		)
	},

	async emergencyAccessRecoveryReminder(
		m: Mailer,
		cfg: Config,
		to: string,
		granteeName: string,
		...
		await send(
			...
			"Pending emergency access request",
			`<p><b>${granteeName}</b> has a pending request for emergency access to <b>${atype}</b> your account. Access will be granted in ${daysLeft} day(s) unless you reject it.</p>`,
			cfg.domain
		)
	},

	async emergencyAccessRecoveryTimedOut(
		m: Mailer,
		cfg: Config,
		to: string,
		granteeName: string,
		...
		await send(
			...
			"Emergency access granted",
			`<p>The wait period for <b>${granteeName}</b>'s emergency access request (${atype}) has elapsed and access has been granted.</p>`,
			cfg.domain
		)
	},
	...
	async twoFactorRemovedFromOrg(m: Mailer, cfg: Config, to: string, orgName: string) {
		await send(
			...
			`<p>You were removed from <b>${orgName}</b> because it requires two-step login on your account.</p>`,
			cfg.domain
		)
	},
	...
	async singleOrgRemovedFromOrg(m: Mailer, cfg: Config, to: string, orgName: string) {
		await send(
			...
			`Removed from ${orgName}`,
			"Removed from organization",
			`<p>You were removed from <b>${orgName}</b> because its policy requires membership in a single organization only.</p>`,
			cfg.domain
		)
	},
	...
	async adminResetPassword(m: Mailer, cfg: Config, to: string, userName: string, orgName: string) {
		await send(
			...
			`<p>The master password for <b>${userName}</b> was recently reset by an administrator of <b>${orgName}</b>. Log in with the new master password given to you.</p>`,
			cfg.domain
		)
	},
	...
	async protectedAction(m: Mailer, cfg: Config, to: string, token: string) {
		await send(
			m,
			to,
			"Your Vaultur Verification Code",
			"Verification code",
			`<p>Your verification code for this protected action is:</p>
<p style="text-align:center;font-size:28px;letter-spacing:6px;font-weight:bold;">${token}</p>
