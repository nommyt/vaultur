...
// ---------------------------------------------------------------------------
// Admin panel config overrides
// ---------------------------------------------------------------------------

/**
 * Runtime config overrides set from the admin panel. Vaultwarden persists these
 * in config.json; on Workers we cannot rewrite env, so the single-row JSON blob
 * here is layered on top of the env-derived defaults (see config-schema.ts).
 * Secrets (ADMIN_TOKEN, JWT_SECRET, push keys) are never stored here.
 */
