...
iconRoutes.get("/:domain/icon.png", async (c) => {
	...
	const cached = await c.env.VAULTUR_KV.getWithMetadata(cacheKey, "arrayBuffer")
	if (cached.value) {
	...
	if (!iconBytes) {
		await c.env.VAULTUR_KV.put(cacheKey, "x", {
			expirationTtl: NEGATIVE_TTL,
			metadata: { miss: true }
		})
	...
	c.executionCtx.waitUntil(
		c.env.VAULTUR_KV.put(cacheKey, iconBytes.bytes, {
