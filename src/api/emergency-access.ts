...
emergencyAccessRoutes.post("/emergency-access/:id/view", async (c) => {
	...
	const db = c.get("db")
	const grantorCiphers = await db.select().from(ciphers).where(eq(ciphers.userUuid, ea.grantorUuid))
...
emergencyAccessRoutes.post("/emergency-access/:id/takeover", async (c) => {
	...
	const grantor = await c.get("db").query.users.findFirst({ where: eq(users.uuid, ea.grantorUuid) })
