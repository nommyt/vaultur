...
import { fromDb, type Cipher, type Folder, type Send } from '@vaultur/db';
...
export class Notify {
  ...
  private async broadcast(userUuid: string, frame: Uint8Array): Promise<void> {
    const id = this.env.NOTIFICATIONS.idFromName(userUuid);
    const stub = this.env.NOTIFICATIONS.get(id);
