 ...
 */
export async function runScheduledJobs(controller: ScheduledController, env: Bindings): Promise<void> {
  ...
  const expired = await db.select({ uuid: sends.uuid, atype: sends.atype }).from(sends).where(lt(sends.deletionDate, nowStr));
  ...
  await db.delete(authRequests).where(and(lt(authRequests.creationDate, authCutoff), sql`${authRequests.approved} IS NULL`));
